...
10. **Narrativa mediante hechos.** `facts.py` prepara frases numéricas con alcance y procedencia. El LLM selecciona IDs y redacta interpretación e hipótesis; el renderer inserta los hechos y limita su cantidad. La presentación usa voz ejecutiva para clientes, periodos en español natural, contexto de negocio y secciones de lectura, hallazgos y consideraciones. Los términos de infraestructura quedan en la trazabilidad, no en la respuesta. Se omite prosa cuantitativa opcional del modelo que duplicaría los hechos. No se sustituyen cifras inventadas por cifras parecidas. La compatibilidad con respuestas en texto sigue pasando por validación.
...
Cuando una comparación entre entidades encuentra filas solo para una de ellas, el resultado conserva esa observación parcial y la respuesta identifica la entidad ausente. No se calcula una diferencia ni se imputan ceros. La interfaz guarda plan y métricas por turno y permite inspeccionar en debug los fallos ocurridos antes de ejecutar una consulta.
